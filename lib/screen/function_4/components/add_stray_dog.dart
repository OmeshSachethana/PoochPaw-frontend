...
class AddStrayDog extends StatefulWidget {
  @override
  _AddStrayDogState createState() => _AddStrayDogState();
}

class _AddStrayDogState extends State<AddStrayDog> {
  XFile? _image;
  ...
  Future<void> _uploadImageToMlAndStrayDogEndpoints(XFile image) async {
    ...
    if (response.statusCode == 200) {
      ...
      final addStrayDogUri = Uri.parse('http://$mlIP:8010/predict/');
      var addStrayDogRequest = http.MultipartRequest('POST', addStrayDogUri)
        ..files.add(await http.MultipartFile.fromPath('file', image.path));

      var addStrayDogStreamedResponse = await addStrayDogRequest.send();
      var addStrayDogResponse =
          await http.Response.fromStream(addStrayDogStreamedResponse);

      if (addStrayDogResponse.statusCode != 200) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(
        title: 'Add Stray Dog Information',
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      SizedBox(height: 10),
                      Text(
                        'Please provide the details of the stray dog you found to help others identify and possibly rescue the dog.',
                        style: TextStyle(
                          fontFamily: 'Nunito',
                          fontSize: 16,
          ...
          : () async {
              ...
              if (_uploadedImageUrl != null) {
                ...
                if (comparisonResults.isNotEmpty &&
                  ...
                  await _firestore.collection('strayDogs').add({
